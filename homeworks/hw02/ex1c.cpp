/// @file
///
/// AERE 5460, Homework 2, Exercise 1c. Solves the heat fin equation
...
/// (`ex1c_M<M>_N<N>.csv`) for each run and puts it in the `data` folder
  ...
  for (double M : M_array) {
    for (std::size_t N : N_array) {
      ...
      const auto output_path =
          base_output_path / std::format("ex1c_M{}_N{}.csv", M, N);
      const auto comment =
